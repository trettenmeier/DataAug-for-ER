...
class Summarizer:
    ...
    def build_index(self):
        ...
        """
        fns = [self.config.train_input_path,
               self.config.val_input_path,
               self.config.test_input_path]
        content = []
        for fn in fns:
            with open(fn, encoding="utf8", errors='replace') as fin:
                for line in fin:
                    LL = line.split('\t')
                    if len(LL) > 2:
                        for entry in LL:
    ...
    def transform_file(self, input_fn, max_len=256, overwrite=False):
        ...
        if not os.path.exists(out_fn) or \
           os.stat(out_fn).st_size == 0 or overwrite:
            with open(out_fn, 'w', encoding="utf8", errors='replace') as fout:
                for line in open(input_fn, encoding="utf8", errors='replace'):
